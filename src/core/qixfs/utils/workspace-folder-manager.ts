...
export class WorkspaceFolderManager {
    ...
    private static addWorkspaceFolder(folder: vscode.WorkspaceFolder): void {
        ...
        const connections   = configuration.get<WorkspaceSetting[]>(`vsQlik.Connection`);
        const connection    = connections?.find(connection => folder.name === connection.name);

        if (connection) {
            const qixWSFolder = new QixWorkspaceFolder(connection, folder.name);
